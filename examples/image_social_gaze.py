...
def _draw_single_person(
    ...
    """Render one person panel with per-person heatmap, bbox, and gaze vector.
    ...
    Args:
        ...
        person_data: Person entry with ``head_location_xyxy``, ``gaze_point_px``,
            ``social_gaze_label``, and optional ``heatmap``.
    ...
    """
    if person_data.get("heatmap") is not None:
        canvas_base = _overlay_heatmap(image.copy(), person_data["heatmap"])
    else:
    ...
    x1, y1, x2, y2 = [int(round(v * scale)) for v in person_data["head_location_xyxy"]]

    gx, gy = [int(round(v * scale)) for v in person_data["gaze_point_px"]]
    ...
    cv2.rectangle(vis, (x1, y1), (x2, y2), color, thickness=1, lineType=cv2.LINE_AA)
    cv2.line(vis, head_center, (gx, gy), color, thickness=1, lineType=cv2.LINE_AA)
    cv2.circle(vis, (gx, gy), 5, color, thickness=-1, lineType=cv2.LINE_AA)
...
def draw_results(image: numpy.ndarray, report: dict[str, Any]) -> numpy.ndarray:
    """Render dyadic social-gaze relation as two stacked per-person panels.
    ...
    Returns:
        Two vertically stacked BGR panels (per-person) or a single fallback
        scene canvas when fewer than two heads were detected.
    """
    ...
    p_panel = _draw_single_person(image, "principal", principal, PRINCIPAL_COLOR, scale)
    a_panel = _draw_single_person(image, "associate", associate, ASSOCIATE_COLOR, scale)

    return _vstack_panels([p_panel, a_panel])
...
def main():
    ...
    out_path = f"{stem}_socialgaze_result{ext}"
    cv2.imwrite(out_path, vis)
    print(f"Saved result to: {out_path}")

    cv2.imshow("social_gaze", vis)
    cv2.waitKey(0)
